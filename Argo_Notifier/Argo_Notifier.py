        ...
        for x in range(0,7):
            for y in range(0,7):
                if len(app.subjects[app.day[x]])-1>=y:
                    if app.debug:
                        print("app.day: {}, x: {}, y: {}, len(app.day[x]): {}, app.subjects[app.day[x]][y]: {}".format(app.day[x],x,y,len(app.day[x]),app.subjects[app.day[x]][y]))
                    self.tableWidget.setItem(y, x, QTableWidgetItem(app.subjects[app.day[x]][y]))
...
class MainApp(QApplication):
    ...
    def __init__(self, *args, **kwargs):
        ...
        try:
            ...
            self.argo_credentials = json.loads(open("credentials/argo_credentials.json", encoding="utf-8").read())
            self.argo_token = json.loads(open("credentials/argo_token.json", encoding="utf-8").read())
            self.calendar_credentials = json.loads(open("credentials/calendar_credentials.json", encoding="utf-8").read())
            self.calendar_token = json.loads(open("credentials/calendar_token.json", encoding="utf-8").read())
        except FileNotFoundError as e:
            print(e)
            print("File Not Found")

        self.day = ["lunedì","martedì","mercoledì","giovedì","venerdì","sabato","domenica"]
    ...
    def update_calendar(self, tests):
        ...
        if not creds or creds.invalid:
            flow = client.flow_from_clientsecrets('credentials/calendar_credentials.json', SCOPES)
